...
def recognize_faces(known_face_encodings, known_face_names):
    ...
    #video_capture = cv2.VideoCapture("http://<địa_chỉ_IP>:<cổng>/video")
    # Ví dụ:
    # video_capture = cv2.VideoCapture("http://192.168.1.100:8080/video")
    # hoặc
    # video_capture = cv2.VideoCapture("http://192.168.1.100:8080/mjpegfeed?640x480") # Nếu cần chỉ định độ phân giải
    #video_capture = cv2.VideoCapture(0)  # Thử 0, 1, 2, ...
    video_capture = cv2.VideoCapture("http://192.168.1.5:4747/video")  # Thay bằng URL RTSP của bạn
